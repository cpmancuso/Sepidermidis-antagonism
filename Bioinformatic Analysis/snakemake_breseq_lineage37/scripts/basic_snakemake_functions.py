...
def split_samplesCSV(PATH_ls,SAMPLE_ls,REF_Genome_ls,PROVIDER_ls,PATIENTID_ls_all):
    ...
    for i, sample in enumerate(SAMPLE_ls):
        ...
        if not(os.path.isdir('0-data/' + sample)):
            os.makedirs('0-data/' + sample, exist_ok=True)
        # check to see if this mini csv with sample info already exists
        if os.path.isfile('0-data/' + sample + '/sample_info.csv'):
            # if so, read file
            old_file = open('0-data/' + sample + '/sample_info.csv','r')
            ...
            if not(old_info == sample_info_csv_text):
                ...
                os.remove('0-data/' + sample + '/sample_info.csv')
                f = open('0-data/' + sample + '/sample_info.csv','w')
...
def makelink(path,sample,providername):
    ...
    subprocess.run('ln -s -T ' + fwd_file + ' 0-data/' + sample + '/R1.fq.gz', shell=True)    
    subprocess.run('ln -s -T ' + rev_file + ' 0-data/' + sample + '/R2.fq.gz', shell=True)    
...
def cp_append_files(paths,sample,providername):
    ...
    for path in paths:
        ...
        print(rev_list)
        print(fwd_list)
    subprocess.run("zcat " + fwd_list + ' | gzip > 0-data/' +  sample + '/R1.fq.gz', shell=True)
    subprocess.run("zcat " + rev_list + ' | gzip > 0-data/' +  sample + '/R2.fq.gz', shell=True)
