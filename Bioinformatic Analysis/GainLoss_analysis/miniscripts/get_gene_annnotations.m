function [ genes_annotations, genes_translations, genes_locustags ] = get_gene_annnotations( start_index, end_index, chr_starts, CDS )
    ...
    if ~isempty( cds_contig ) % if contig has genes
        ...
        if ~isempty( cds_contig_index ) % if position is on a coding region
            if cds_contig_index~= last_cds_contig_index % if this coding region is a new gene that hasn't already been recorded
                ...
                % Record locustag
                genes_locustags{end+1} = cds_contig(cds_contig_index).locustag(end-4:end);
...
genes_locustags = genes_locustags(unique_genes);
