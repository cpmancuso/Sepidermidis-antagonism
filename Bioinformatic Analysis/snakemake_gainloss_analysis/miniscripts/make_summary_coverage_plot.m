...
abs_max_all = max(max(abs(coverage_matrix(:,plotstart:plotend))));
abs_max_inner = max(max(abs(coverage_matrix(:,start_genome_index:end_genome_index))));
pad=3;
if isequal( matrix_type, 'raw coverage')
    ylim([ 0 1.1*min(abs_max_all,pad*abs_max_inner) ])
...
elseif isequal( matrix_type, 'normalized coverage' )
    ...
    %try to adjust ylim
    ytest=[ -1.1*min(abs_max_all,pad*abs_max_inner) 1.1*min(abs_max_all,pad*abs_max_inner) ];
    if ytest(1)< ytest(2) %weird bug with 0s
        ylim([ -1.1*min(abs_max_all,pad*abs_max_inner) 1.1*min(abs_max_all,pad*abs_max_inner) ])
    end
else
